...
pve2's `local-lvm` is a thin pool provisioned well beyond its physical volume group: the sum of guest virtual disks exceeds the VG size, with roughly 16 GiB of free VG space. At around 20% pool usage this is not an immediate risk, but it constrains how guests may change. Guest disks must not be grown without first adding physical storage — a resize increases provisioning pressure on a pool that is already overcommitted, the wrong direction. And a runaway guest consumes real pool space quickly; if the pool itself reaches 100%, writes fail across every guest on it, a worse failure than a single guest filling. `thin_pool_autoextend_threshold`/`_percent` are set (80/20) as a rail, but with only ~16 GiB of VG headroom that rail is short. The real remedy is more physical disk in pve2; until then, guest sizing is fixed.
